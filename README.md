...
RPS: варьируется от 600 до 1300 в зависимости от количества воркеров и длительности теста.
Одна оптимизация есть на ветке sql-optimizations. Это оптимизация CreatePullRequest.
Благодаря ей есть небольшой прирост в скорости, но мне не хотелось переписывать много строк кода ради небольшого прироста.
Для теста использовался стресс-тестер в `cmd/stresser`.

Для профилировки использовался pprof.
